...
beforeEach(() => {
    const newsFile = JSON.parse(
        fs.readFileSync('tests/integration/testData/news.json', {
            encoding: 'utf8',
            flag: 'r',
        })
    )
    jest.spyOn(WordpressService.prototype, 'getNews').mockResolvedValue(
        newsFile
    )

    const tnFile = JSON.parse(
        fs.readFileSync('tests/integration/testData/tournament-news.json', {
            encoding: 'utf8',
            flag: 'r',
        })
    )
    jest.spyOn(
        ...
        'getTournamentNews'
    ).mockResolvedValue(tnFile)

    const ccFile = JSON.parse(
        fs.readFileSync('tests/integration/testData/content-creators.json', {
            encoding: 'utf8',
            flag: 'r',
        })
    )
    jest.spyOn(
        WordpressService.prototype,
        'getContentCreators'
    ).mockResolvedValue(ccFile)

    const ftFile = JSON.parse(
        fs.readFileSync('tests/integration/testData/faf-teams.json', {
            encoding: 'utf8',
            flag: 'r',
        })
    )
    jest.spyOn(WordpressService.prototype, 'getFafTeams').mockResolvedValue(
        ftFile
    )

    const nhFile = JSON.parse(
        fs.readFileSync('tests/integration/testData/newshub.json', {
            encoding: 'utf8',
            flag: 'r',
        })
    )
    jest.spyOn(WordpressService.prototype, 'getNewshub').mockResolvedValue(
        nhFile
    )
    ...
    jest.spyOn(
        ...
        if (id in mapping) {
            return JSON.parse(
                fs.readFileSync(
                    'tests/integration/testData/leaderboard/' +
                        mapping[id] +
                        '.json',
    ...
    nock(appConfig.apiUrl)
        ...
        .reply(
            200,
            fs.readFileSync('tests/integration/testData/clan/clan.json', {
                encoding: 'utf8',
                flag: 'r',
            })
        )
